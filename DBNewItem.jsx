...
const DBNewItem = () => {
  ...
  return (
    <div className='flex items-center justify-center flex-col pt-6 px-24 w-full gap-2'>
       ...
       <div className='w-full flex items-center justify-around gap-3 flex-wrap'>
        {statuses && statuses?.map(data => (
          <p
           ...
           className={`px-4 py-3 rounded-md text-xl bg-gray-600 text-teal-50 font-semibold cursor-pointer hover:shadow-md border-gray-200 backdrop-blur-md 
         ...
         <div className='w-full bg-card backdrop-blur-md h-370 rounded-md border-2 border-dotted border-gray-300 cursor-pointer'>
          {isLoading ?
           <div className='w-full h-full flex flex-col items-center justify-evenly px-24'>
            <Spinner />
            <div class="mb-1 text-base font-medium text-red-700 dark:text-red-500">{Math.round(progress)}%</div>
<div class="w-full bg-gray-200 rounded-full h-2.5 mb-4 dark:bg-gray-700">
<div class="bg-red-600 h-2.5 rounded-full dark:bg-red-500" style={{ width: `${progress}%` }}></div>
</div>
           </div> : 
           <>{!imageDownloadURL ? <>
           <label>
         ...
         <motion.div onClick={submitNewData} {...buttonClick} className='w-9/12 py-2 rounded-md bg-red-400 text-primary hover:bg-red-500 cursor-pointer text-center'>
          Save
         </motion.div>
      </div>
    </div>
